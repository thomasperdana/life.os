...
> **Written:** 2026-08-29 · **Spec version:** 2.0.0 (supersedes [SPEC.1.md](SPEC.1.md))
> **Repo:** `/Volumes/181TB/Developers-LLC/life.os` · **Source request:** [note.md](note.md)
>
> **v2 changes:** concrete P0 toolchain steps for this Mac Mini M4 (§4.7, §16); Google Antigravity
> named as the coding editor, Xcode as the required compiler/signer (§5.1); Apple Developer Program
> cost made explicit at the phase that actually needs it (§16).

---

## 1. Context & Problem

The five-step Bible study method in [note.md](note.md) already runs today — as prose prompts in the `kjv9`, `kjv71`, `kjv72`, `kjv81`, and `kjv82` skills, and in the prompt lineage at `/Volumes/182TB/northseminole.gideon/archive/1.prompt.kjv*.md`. It works, and it produces genuinely good study material. But it only runs where Claude Code runs: a Mac, a terminal, a subscription.
...
`note.md` sketches a granularity ladder — **word → phrase → sentence → verse → chapter → book** — read top-to-bottom, left-to-right. The app makes that ladder a first-class control. The aperture is inferred from the input and adjustable afterward:
...
| **Verse** | `John 3:16` | The default shape, exactly as `note.md` describes it |
...
Verified: `xcode-select` points at `/Library/Developer/CommandLineTools`. Swift 6.3.3 is present, targeting `arm64-apple-macosx26.0`. No iOS SDK. No simulators. `xcodebuild` is unavailable. The Mac Mini M4 hardware itself is not the constraint — Xcode simply isn't installed yet.

> **Consequence.** Build phase P0 is "install Xcode" (concrete steps in §16). It's a free App Store download plus a free Apple ID sign-in — no paid account needed until P6's TestFlight step (§16). No claim about iOS compilation, simulator behavior, or app appearance can be made from this repo until Xcode is installed.
...
### 5.1.1 Editor vs. compiler (Mac Mini M4, Google Antigravity)

Two different tools do two different jobs, and neither substitutes for the other:

| Tool | Role | Required? |
|---|---|---|
| **Google Antigravity** | Writes and edits the Swift/SwiftUI/SQL source | Optional — any editor works; Antigravity is fine for this |
| **Xcode** | Compiles, links, code-signs, runs the simulator, submits builds | **Mandatory** — no substitute exists on iOS |

Antigravity can produce every file this SPEC describes. It cannot build an `.ipa`, launch a simulator, or put a build on a physical iPhone — only Xcode's toolchain (`xcodebuild`, `simctl`, the code-signing machinery) does that. A Mac Mini M4 runs both without strain; the constraint is Xcode's presence, not the machine's power. See §16 P0 for the exact install steps and §4.7 for what's confirmed absent on this machine today.
...
|---|---|---|
| KJV text | `seven1m/bible_api`, `wldeh/bible-api`, `jadenzaleski/bible-translations`, `Beblia/Holy-Bible-XML-Format` (all four already collected in `note.md`); `scrollmapper/bible_databases` | ☐ licence read · ☐ verse count = 31,102 · ☐ spot-check 20 verses against a print KJV |
...
The five steps come verbatim in intent from `note.md`. Each subsection below names: **purpose · grounding payload · response schema · model rung · verification rule · failure mode.**
...
P0 is blocking and is not satisfied today (§4.7). It costs nothing but time and disk:

1. **Install Xcode** — Mac App Store → Xcode (~15 GB, current stable release). The Mac Mini M4 has ample headroom; this is a download-time constraint, not a hardware one.
2. **Accept the license and let Xcode install its additional components** the first time it launches (`xcodebuild -runFirstLaunch` if doing this from Terminal).
3. **Point the active toolchain at it**: `sudo xcode-select -switch /Applications/Xcode.app/Contents/Developer` — this is the step that was missing on this machine (§4.7 found the CLT-only path still active).
4. **Sign in with a free Apple ID** in Xcode → Settings → Accounts. This alone is enough for P0–P5: build, run in the simulator, and even install on your own iPhone via personal-team signing (7-day reinstall expiry, no TestFlight).
5. **Verify**: `xcodebuild -showsdks` should now list an iOS SDK, and File → New → App → Run should launch an empty app in the simulator.

**P6 has one additional cost the free Apple ID does not cover:** the **Apple Developer Program, $99/year**, required for TestFlight distribution to testers beyond your own signed-in devices, and later for any App Store release (§17 Q1). Nothing before P6 needs it — defer the $99 until the app is actually ready to leave your own device.

**Where Google Antigravity fits:** write and edit every Swift/SwiftUI/SQL file in Antigravity if that's the preferred editor (§5.1.1) — Xcode is still what runs P0's verification step and every `xcodebuild`/simulator/signing action in P1–P6. The two tools are not alternatives; Antigravity edits, Xcode builds.
...
5. **Constable's Expository Notes** (`soniclight.com`, linked in `note.md`, and named as source #3 in the original BOS note). Freely available for personal use; bundling them in a distributed app is a different question. Needs a written permission request before any inclusion.
...
7. **The aithesaurus.io theology thesaurus** (in `note.md`) — a possible topic-expansion source. Unexamined.
...
| [note.md](note.md) | The request, the aperture ladder, and the four Bible-data repositories. |
